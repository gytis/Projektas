...
from tinklas.forms.forms import PridetiFoto
...
def asmenine_info(request):
    if request.user.is_authenticated():
        user = request.user     
        return render_to_response("info/info.html", {'user': user})
    else:
...
def vartotojo_info(request):
    if request.user.is_authenticated():
        ...
        if username is not None and username != request.user.username:
            ...
            return render_to_response('info/user_info.html', 
            ...
            if form.is_valid():
                if len(User.objects.filter(email=request.POST['El_pastas'])) <= \
                    ...
                    if request.POST['Slaptazodis'] == \
                        ...
                        return render_to_response("info/duomenu_keitimas.html", 
...
def keisti_slaptazodi(request):
    if not request.user.is_authenticated():
            ...
            if form.is_valid():
                if user.check_password(request.POST['Senas_slaptazodis']):
                    if request.POST['Naujas_slaptazodis'] == \
                        ...
                        user.save()
                        return render_to_response('info/keitimas.html', {'message': "Slaptazodis pakeistas"})
                    else:
        ...
        return render_to_response('info/keitimas.html', {'form': form, 'error': error})
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                if user.check_password(request.POST['Slaptazodis']):
                    ...
                    profilis.save()
                    return render_to_response('info/keitimas.html', {'message': "Nuotrauka pakeista"})
                else:
